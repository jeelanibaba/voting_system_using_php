...
<?php 
    }else if(isset($_GET['delete_id']))
    {
        ...
        mysqli_query($db, "DELETE FROM votings WHERE id = '". $d_id ."'") OR die(mysqli_error($db));
...
<?php 
    ...
    if(isset($_POST['addElectionBtn']))
    {
        ...
        if (strtotime($starting_date) >= strtotime($current_date) && strtotime($ending_date) >= strtotime($current_date)) {
            $status = "Active";
        } else {
            $status = "InActive";
        }
